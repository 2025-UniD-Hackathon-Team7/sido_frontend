...
function Paragraph1({ reward }: { reward: number }) {
  return (
    <div className="h-[32px] relative shrink-0 w-full" data-name="Paragraph">
      <p className="absolute font-['Arimo:Regular',sans-serif] font-normal leading-[32px] left-0 text-[#bb4d00] text-[24px] top-[-2.84px] w-[44px]">+{reward}</p>
    </div>
  );
}

function Container1({ reward }: { reward: number }) {
  return (
    <div className="absolute bg-[#FFF7ED] box-border content-stretch flex flex-col gap-[7.997px] h-[99.978px] items-start left-0 pb-0 pt-[15.993px] px-[15.993px] rounded-[16.4px] top-0 w-[160px]" data-name="Container">
      <Container />
      <Paragraph1 reward={reward} />
    </div>
  );
}
...
function Text3() {
  return (
    <div className="h-[15.993px] relative shrink-0 w-[45px]" data-name="Text">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid box-border content-stretch flex h-[15.993px] items-start relative w-[45px]">
        <p className="basis-0 font-['Arimo:Regular','Noto_Sans_KR:Regular',sans-serif] font-normal grow leading-[16px] min-h-px min-w-px relative shrink-0 text-[12px] text-neutral-600 whitespace-nowrap">경험치</p>
      </div>
    </div>
  );
}
...
function Paragraph2({ xp }: { xp: number }) {
  return (
    <div className="h-[32px] relative shrink-0 w-full" data-name="Paragraph">
      <p className="absolute font-['Arimo:Regular',sans-serif] font-normal leading-[32px] left-0 text-[#8200db] text-[24px] top-[-2.84px] w-[44px]">+{xp}</p>
    </div>
  );
}

function Container3({ xp }: { xp: number }) {
  return (
    <div className="absolute bg-[#FFF7ED] box-border content-stretch flex flex-col gap-[7.997px] h-[99.978px] items-start left-[172px] pb-0 pt-[15.993px] px-[15.993px] rounded-[16.4px] top-0 w-[160px]" data-name="Container">
      <Container2 />
      <Paragraph2 xp={xp} />
    </div>
  );
...
function Container4({ reward, xp }: { reward: number; xp: number }) {
  return (
    <div className="h-[99.978px] relative shrink-0 w-full" data-name="Container">
      <Container1 reward={reward} />
      <Container3 xp={xp} />
    </div>
  );
}

export default function RewardContainer({ reward, xp }: { reward: number; xp: number }) {
  return (
    <div className="relative rounded-[16px] size-full" data-name="Container">
      ...
      <div className="size-full">
        <div className="box-border content-stretch flex flex-col gap-[15.993px] items-start pb-[0.843px] pt-[24.833px] px-[24.833px] relative size-full">
          <Paragraph />
          <Container4 reward={reward} xp={xp} />
        </div>
      </div>
    </div>
  );
